...
// Service
import {createNewLoan} from '../Services/borrowService';
// Dispatch
import {updateMessage} from '../../Redux/snackBarMessageSlice';

const ConfirmBorrowDetails = props => {
  ...
  const handleBorrowRequestConfirm = async () => {
    setisLoading(true);
    const response = await createNewLoan(jwtToken, {
        ...
        <Card style={styles.cardView}>
          ...
          <Card.Content style={{marginTop: 17}}>
            ...
            <View style={{flexDirection: 'row'}}>
              ...
              <Text style={{fontSize: 17}}>{params.rate} </Text>
            </View>
            <View style={{flexDirection: 'row'}}>
            ...
            <View style={{flexDirection: 'row'}}>
              <Text style={styles.textView}>Collateral Token : </Text>
              <Text style={{fontSize: 17}}>{params.collateral}</Text>
            </View>
          </Card.Content>
        </Card>
      </View>
          ...
          <Button
            ...
            onPress={() => {
              if (!isConfirm) {
                ...
                setIsConfirm(false);
                handleBorrowRequestConfirm();
              }
            }}>
            {!isLoading && (isConfirm ? 'Click again to - CONFIRM' : 'Borrow')}
          </Button>
        )}
      </View>
...
export default ConfirmBorrowDetails;
